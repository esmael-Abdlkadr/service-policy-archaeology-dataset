...
The construction procedure is documented in full in `DATASET_DESCRIPTION.md`: the usage and meter-reading model, the policy menus and component-type tendencies, the parent coupling rule, the operational noise (late completion, batched visits, silent skips), the rules that keep every label recoverable, and the operator-held-out split.
